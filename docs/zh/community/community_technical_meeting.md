# 社区技术会议

Triton-Ascend 定期召开社区技术会议，作为项目治理的核心决策机制。技术方向、重大提案评审和版本发布等重大事项在技术会议中讨论和决定，由全体 `维护者` 投票。会议对所有社区成员开放，任何人均可参与讨论，但决策和投票仅由 `维护者` 进行。

## 会议内容

- 技术方向讨论与决策
- 重大提案（RFC）评审与投票
- 版本发布规划与时间线
- 社区 Issue 和 PR 优先级排序
- 贡献者提名与移除讨论

## 会议安排

- **会议频率**：每两周一次
- **会议时间**：周四 14:15（UTC+8）
- **会议起始日期**：2026 年 6 月 4 日
- **会议链接**：[链接](https://us06web.zoom.us/j/86540192059?pwd=X2N3pl5bnZH5CgiuX1zjyZoELItmjP.1)
- **会议室号**：865 4019 2059
- **会议室密码**：123456
- **会议议程与纪要**：[议程&纪要](https://docs.google.com/document/d/1qfat2wZtO2nfZb5FC2dWAcR6sTqgTNSvvh7MzTDoI4s/edit?pli=1&tab=t.0)
- **会议 iCalendar**：[iCalendar-ics](https://us06web.zoom.us/meeting/tZIoc-mppzkqH92hP7TBYHppsqZUxg9F8VPf/ics?icsToken=DObLUj7lyTNwJj6XJgAALAAAAOvNV2DJnuObI9tqQGt1c3aeqVgCdzMYHV0m_J0KHIX0i4QUrtvsJwMu_EMF5VKyBKdX4rPWh_raTxvF4jAwMDAwMQ&meetingMasterEventId=1PZVSVC8S8WIBZJ1e4wuZQ)

## 投票规则

- 未出席会议的 `维护者` 须在会议后 3 个工作日内提交投票；逾期未投票视为弃权
- 投票通过条件为参与投票的 `维护者` 超过半数同意（弃权不计入投票总数）
- 投票结果记录于会议纪要中

## 决策流程

### 常规决策

日常开发中的常规决策（如 `Bug` 修复、小功能改进）由相关模块的 `维护者` 在 `Issue` 或 `PR` 中审阅决定，无需召开会议。

### 重大决策

以下事项视为重大决策，需由 `维护者` 在社区技术会议中集体讨论：

- 重大架构变更
- 破坏性 `API` 变更
- 新增核心功能或移除现有功能
- 第三方依赖的重大升级
- 版本发布规划

重大决策流程：

1. 提交 `RFC`（征求意见稿），描述变更内容、影响范围和实施方案
2. 在技术会议中讨论
3. `维护者` 共同决策
